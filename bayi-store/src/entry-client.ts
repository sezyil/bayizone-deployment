 ...
 */
createApp().then(async (bayizone) => {
    // @ts-ignore
    const initialState = window.__bayizone__
    // restore pinia state from SSR if any
    if (typeof initialState?.pinia === 'object') {
        bayizone.pinia.state.value = initialState.pinia
    }

    // wait for the app to be ready
    await bayizone.router.isReady()

    // finaly mount the app to the DOM
    bayizone.app.mount('#app')
})
