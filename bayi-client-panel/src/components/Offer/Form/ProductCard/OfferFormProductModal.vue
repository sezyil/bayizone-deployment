...
import { IProductVariantColorData } from '/@src/shared/product/interface-product';
...
export interface IPropsOfferFormProductModal {
    productIndex: number | null;
    active: boolean;
    /* IOfferFormProduct[] */
    productData: IOfferFormProduct[];
...
const props = defineProps<IPropsOfferFormProductModal>();

const swal = useSwal();
const resetFormData = () => {
    formData.value = { ...addOfferFormLine() };
    colorForm.value = [];
    dimensionForm.value = [];
}
...
const formData = ref<IOfferFormLine>({ ...addOfferFormLine() });
...
const selectedCurrency = computed(() => {
    if (store.offerForm.detail.currency === "0") return "";
    return getCurrencySymbol(store.offerForm.detail.currency);
});
...
watch(() => formData.value.product_id, (value) => {
    ...
    if (value === 0) {
        ...
        if (findProduct) {
            ...
            let _selectedCurrency = store.offerForm.detail.currency;
            ...
            if (props.productIndex !== null) {
                if (store.offerForm.lines[props.productIndex]) {
                    if (store.offerForm.lines[props.productIndex].product_id === value) {
                        formData.value = { ...store.offerForm.lines[props.productIndex] }
                            ...
                            <div class="col-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" v-model="formData.unit_price" class="form-control" />
                                    </div>
                                </div>
