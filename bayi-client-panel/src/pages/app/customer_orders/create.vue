...
viewWrapper.setPageTitle('Sipariş Oluştur');
...
const submitForm = async (e: Event) => {
    ...
    if (!store.orderForm.detail.order_date) errors.push('Sipariş tarihi seçiniz.');
    ...
    try {
        ...
        swal.fire({
            title: 'Başarılı',
            html: 'Sipariş başarıyla oluşturuldu.',
            icon: 'success',
            confirmButtonText: 'Tamam',
        });
        store.$reset();
...
<template>
    <PageContent>
        <template #header>
            <!-- save button -->
            <VButtonSave @click="triggerSubmit" />
        </template>
