...
viewWrapper.setPageTitle('Proforma Oluştur');
...
const requestId = ref()
const checkQuery = () => {
    const request_id = router.currentRoute.value.query.request_id;
    if (request_id) {
        store.setOfferRequestMode(true)
...
const submitForm = async (e: Event) => {
    ...
    try {
        ...
        const response = await api.add(_form, requestId.value);
        swal.fire({
            title: 'Başarılı',
            html: 'Proforma başarıyla oluşturuldu.',
            icon: 'success',
            confirmButtonText: 'Tamam',
        });
        store.$reset();
...
<template>
    <PageContent>
        <template #header>
            <!-- save button -->
            <VButtonSave @click="triggerSubmit" type="button" />
        </template>

        <form id="offer_form" @submit.prevent="submitForm" v-if="rendered">
            <!-- hidden btn for click event -->
            <button type="submit" class="d-none" ref="triggerBtn"></button>
            <OfferForm :request-id="requestId" />
        </form>
    </PageContent>
</template>
