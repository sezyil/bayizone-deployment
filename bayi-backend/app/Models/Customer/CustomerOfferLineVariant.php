...
class CustomerOfferLineVariant extends Model
{
    use HasFactory;

    protected $fillable = [
        'customer_offer_line_id',
        'type',
        'product_variant_id',
        'product_variant_value_id',
    ];

    public function customer_offer_line()
    {
        return $this->belongsTo(CustomerOfferLine::class);
    }
    ...
    public function transformData(bool $onlyValue = false, $withDetail = false)
    {
        if ($onlyValue) {
            $data = [
                'id' => $this->id,
                'customer_offer_line_id' => $this->customer_offer_line_id,
                'type' => $this->type,
